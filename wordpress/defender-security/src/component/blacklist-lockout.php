...
class Blacklist_Lockout extends Component {
	...
	public function output_scripts_data( $data ) {
		...
		if ( $exist_geodb ) {
			...
			$blacklist_countries = array_merge( array( 'all' => __( 'Block all', 'defender-security' ) ), $country_list );
			$whitelist_countries = array_merge( array( 'all' => __( 'Allow all', 'defender-security' ) ), $country_list );
		} else {
