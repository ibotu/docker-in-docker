...
function display_meta_shortcode() {
    global $post;
    $output = '<ul>';
    ...
    foreach ($meta_values as $key => $value) {
        if (!empty($value[0])) {
            $output .= '<li><strong>' . esc_html(ucwords(str_replace('_', ' ', $key))) . ':</strong> ' . esc_html($value[0]) . '</li>';
        }
    }
...
function handle_form_submission() {
    $post_id = wp_insert_post(array(
        ...
        'post_type' => 'post', // Change this to the post type you want
        'post_status' => 'publish', // Or 'draft' if you want to review it before publishing
    ));

    if (!is_wp_error($post_id)) {
        // The post was created successfully, add the meta fields
        add_post_meta($post_id, 'profile_name_meta_box', sanitize_text_field($_POST['profile_name_meta_box']));
...
add_action('admin_post_add_custom_post', 'handle_form_submission');
add_action('admin_post_nopriv_add_custom_post', 'handle_form_submission');
...
function register_custom_meta() {
	...
	foreach ($meta_boxes as $meta_box) {
		register_meta('post', $meta_box, array('show_in_rest' => true));
	}
}
...
function enqueue_my_script() {
    ...
    wp_localize_script('jquery', 'my_script_vars', array(
        'nonce' => $nonce,
    ));
}
...
function my_custom_endpoint($request) {
    $args = array(
        ...
        'meta_query' => array(
            array(
                'key' => 'profile_name_meta_box',
                'value' => $request['profile_name_meta_box'],
                'compare' => 'LIKE'
            )
        )
    );
